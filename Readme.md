...
-IdTask - хранит уникальный индентификатор задачи.
- Id - устанавливает и возвращает уникальный индентификатор.
...
TaskEnhancer - это абстрактный базовый класс для всех декораторов задач. Он включает в себя общие атрибуты и методы, такие как Task,  Title, Description, StateTask, Id, Info() и CompleteTask().

## Интерфейс ITask

ITask - это интерфейс, который определяет общие атрибуты и методы для всех классов задач. Он включает в себя Id, Title, Description, StateTask, Info() и CompleteTask().
